...
@Component
public class RelativeNewPersonsThenMultiCooccurrenceAiSearchPlanner {
    ...
    public AiSearchPlan plan(String query, List<Long> anchorPersonIds, List<String> anchorPersonNames, int offset, int limit) {
        ...
        return planForYears(query, targetYear, anchorPersonIds, anchorPersonNames, offset, limit);
    }
    ...
    public AiSearchPlan planForYears(String query,
        ...
        plan.setPlanType("relative_new_persons_then_multi_cooccurrence");
        ...
        plan.getMetadata().put("intent", "relative_new_persons_then_multi_cooccurrence");

        plan.getSteps().add(yearFilterStep("target_range_photos", "筛选目标年份中的可见照片", targetStart, targetEnd));
        plan.getSteps().add(endBoundFilterStep("baseline_photos", "筛选目标年份之前的可见照片", baselineEnd));
        ...
        derive.setId("relative_new_persons_multi_cooccurrence");
        derive.setOperator("derive_temporal_person_cooccurrence");
        derive.setInputRef("new_persons");
        derive.setOutputKey("relative_new_persons_multi_cooccurrence");
        derive.getDependsOn().add("new_persons");
        derive.setDescription("统计这些新认识人物在首次出现之后与多位锚点人物共同出现的频率");
        ...
        sort.setId("sorted_relative_new_persons_multi_cooccurrence");
        sort.setOperator("sort");
        sort.setInputRef("relative_new_persons_multi_cooccurrence");
        sort.setOutputKey("sorted_relative_new_persons_multi_cooccurrence");
        sort.getDependsOn().add("relative_new_persons_multi_cooccurrence");
        sort.setDescription("按后续多锚点同框频次与最近同框时间排序");
        sort.setArgs(Map.of(
        ...
        limitStep.setId("limited_relative_new_persons_multi_cooccurrence");
        limitStep.setOperator("limit");
        limitStep.setInputRef("sorted_relative_new_persons_multi_cooccurrence");
        limitStep.setOutputKey("limited_relative_new_persons_multi_cooccurrence");
        limitStep.getDependsOn().add("sorted_relative_new_persons_multi_cooccurrence");
        limitStep.setDescription("分页截断多锚点后续同框结果");
    ...
    private AiSearchPlanStep yearFilterStep(String id, String description, String startDate, String endDate) {
        ...
        step.setArgs(Map.of("startDate", startDate, "endDate", endDate));
        return step;
    }
