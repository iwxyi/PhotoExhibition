...
@Component
public class RelativeNewPersonsThenCooccurrenceMissingAgainAiSearchPlanner {
    ...
    public AiSearchPlan plan(String query, Long anchorPersonId, String anchorPersonName, int offset, int limit) {
        ...
        return planForYears(query, targetYear, currentYear, anchorPersonId, anchorPersonName, offset, limit);
    }
    ...
    public AiSearchPlan planForYears(String query,
        ...
        plan.setPlanType("relative_new_persons_then_cooccurrence_missing_again");
        ...
        plan.getMetadata().put("intent", "relative_new_persons_then_cooccurrence_missing_again");
        ...
        derive.setId("new_persons_cooccurrence");
        derive.setOperator("derive_temporal_person_cooccurrence");
        derive.setInputRef("new_persons");
        derive.setOutputKey("new_persons_cooccurrence");
        derive.getDependsOn().add("new_persons");
        derive.setDescription("统计新认识人物在首次出现后与锚点人物的后续同框频率");
        ...
        missingAgain.setId("cooccurring_new_persons_missing_again");
        missingAgain.setOperator("set_difference");
        missingAgain.setInputRef("new_persons_cooccurrence");
        missingAgain.setOutputKey("cooccurring_new_persons_missing_again");
        missingAgain.getDependsOn().add("new_persons_cooccurrence");
        missingAgain.getDependsOn().add("missing_again_year_persons");
        missingAgain.setDescription("剔除今年又出现的人物，保留后来与锚点同框但今年没再出现的人");
        ...
        sorted.setId("sorted_cooccurring_new_persons_missing_again");
        sorted.setOperator("sort");
        sorted.setInputRef("cooccurring_new_persons_missing_again");
        sorted.setOutputKey("sorted_cooccurring_new_persons_missing_again");
        sorted.getDependsOn().add("cooccurring_new_persons_missing_again");
        sorted.setDescription("按后续同框频次与最近同框时间排序");
        ...
        limited.setId("limited_cooccurring_new_persons_missing_again");
        limited.setOperator("limit");
        limited.setInputRef("sorted_cooccurring_new_persons_missing_again");
        limited.setOutputKey("limited_cooccurring_new_persons_missing_again");
        limited.getDependsOn().add("sorted_cooccurring_new_persons_missing_again");
        limited.setDescription("分页截断后来同框但今年没再出现的人物结果");
